  ...
  return Object.keys(s).reduce((e, n) => {
    ...
    switch (a) {
      ...
      case P.Number:
        ...
        break;
      default:
        if (typeof a == "object" && t.type === "struct")
          ...
          Array.isArray(a) && t.type === "array" ? e[n] = t.value.map((o) => u(a[0], o)) : e[n] = t.value;
        break;
    }
    return e;
  }, {});
...
var F = async (s, i, e, n = "FixedLen", a = 1e3) => {
  let t = 0, o = true;
  for (; o; ) {
    let d = e ? { Keys: { keys: "HashedKeys" in e ? e.HashedKeys : e.Keys.keys, pattern_matching: n, models: [...i.map((m) => m.metadata?.name)] } } : null, c = await s.getEntities({ limit: a, offset: t, clause: d || void 0 });
...
var l = async (s, i) => {
  for (let e in s)
    if (Object.hasOwn(s, e))
      for (let n in s[e]) {
        if (!Object.hasOwn(s[e], n))
          continue;
        let a = Object.values(i).find((t) => t.metadata?.namespace + "-" + t.metadata?.name === n);
        if (a)
          try {
            E(a, e, u(a.schema, s[e][n]));
          } catch (t) {
            console.warn(`Failed to set component ${a.metadata?.name} on ${e}`, t);
          }
      }
};
export {
