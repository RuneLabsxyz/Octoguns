...
pub mod actions {
    ...
    use octoguns::lib::helpers::{
        get_character_ids, get_character_positions, get_all_bullets, check_is_character_owner,
        filter_out_dead_characters, extract_bullet_ids, check_win, check_is_valid_move
    };
    ...
    impl ActionsImpl of IActions<ContractState> {
        fn move(ref self: ContractState, session_id: u32, mut moves: Array<TurnMove>) {
            ...
            match session_meta.turn_count % 2 {
                0 => { assert!(player == session.player2, "not turn player"); },
                1 => { assert!(player == session.player1, "not turn player"); },
                _ => {}
            }
            ...
            while step_count < 99_u8 {
                ...
                loop {
                    ...
                    let mut next_bullet_shot = *character_move.shots.at(0);

                    if bullet_index < character_move.shots.len() {
                        next_bullet_shot = *character_move.shots.at(bullet_index);
                    }
                    ...
                    loop {
                        ...
                        if !is_collision {
                            ...
                            if movement.xdir {
                                new_x =
                                    ((character.coords.x + 100).try_into().unwrap()
                                        + movement_y * 2);
                            } else {
                                new_x =
                                    ((character.coords.x + 100).try_into().unwrap()
                            ...
                            if movement.ydir {
                                new_y =
                                    ((character.coords.y + 100).try_into().unwrap()
                                        + movement_x * 2);
                            } else {
                                new_y =
                                    ((character.coords.y + 100).try_into().unwrap()
                ...
                let (new_bullets, dead_characters) = simulate_bullets(
                    ref bullets, ref all_character_positions
                );

                // Update models in the world
                let (new_user_character, new_user_character_ids) = filter_out_dead_characters(
                    world, @all_character_positions, dead_characters.clone()
                );

                // Remove dead characters from all_character_ids
                let (new_all_character, new_all_character_ids) = filter_out_dead_characters(
                    world, @all_character_positions, dead_characters.clone()
                );
